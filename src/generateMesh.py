...
def generateMesh(model, showGmshMesh=False,showGmshGeometryBeforeMeshing = False, 
                 elementDefinition='MITC-4-N', meshSize=6e-1, nEdgeNodes=0, 
                 order='linear', meshDistortion = False, distVal = 100, deactivateRotation=False):
    ...
    if elementShape == 4 or elementShape == 9 :
        ...
        for i in range(0, len(model.plates)):
            gmsh.model.geo.mesh.setRecombine(2, model.plates[i].tag)
    elif elementShape != 3:
        raise Exception
     # (1: MeshAdapt, 2: Automatic, 3: Initial mesh only, 5: Delaunay, 
     # 6: Frontal-Delaunay (default), 7: BAMG, 
    ...
    nodeTagsModel , nodeCoords = gmsh.model.mesh.getNodes()[:2]

    nodesArray = np.array(nodeCoords).reshape((-1,3))
    # Why a dataframe? Nodes are not always continuous since 
    # sometimes are removed with removeDuplicateNodes. 
    # Using a pandas dataframe allows to uniquely call a 
    # node with his tag
    ...
    elementsList, getElementByTagDictionary = _getElementsList(
        gmshModel, platesList, elementType, elementShape,
        elementIntegration, gmshToCoherentNodesNumeration, 
        nodesArrayPd
    )
    # useful if there is the need to distinguish plate elements from 
    # the downstand beam elements
...
def setMesh(model, nodesArray, elements, BCs,elementDefinition = None, load = None):
    ...
    * **load = None**: n x 4 numpy array. n is the number of loaded nodes, the first column is the node tag, the other column represent the magnitude of the load for the relative DOF. 
    ...
    model.mesh = _Mesh(nodesArrayPd,nodesRotationsPd, elementsList, BCs, [], elementsList, [],[])
    model.mesh.load = load
...
def _getElementsList(gmshModel, platesList, elementType, 
                     elementShape, elementIntegration, 
                     gmshToCoherentNodesNumeration, nodesArrayPd):
...
def _createNewNodesForDownStandBeams(gmshModel,nodesArray,nodesArrayPd,
                                     nodesRotationsPd,downStandBeamsList,
                                     gmshToCoherentNodesNumeration,nextNode):
...
def _getDownStandBeamsElements(gmshModel, nodesRotationsPd, nodesArray, elementsList, 
    ...
    Additionally, creates the multi-constraint matrix which connects plate and beam DOFs.
        Input: 
            * gmshModel: model object of the gmsh library.
            * nodesRotationsPd: pandas dataframe where the indexes are the node tags, the values are 
              the node rotation in radians
            * nodesArray: nNodes x 3 numpy array. Columns are the x-y-z coordinates of each node.
            ...
            * gmshToCoherentNodesNumeration: Pandas dataframe where the indexes are the node tags 
              assigned by gmsh, the values are an array from 0 to nNodes-1.
            * nodesArrayPd: Pandas Dataframe where the indexes are the node tags 
              assigned by gmsh, the values are a nNodes x 3 array with the x-y-z coordinates.
        Return: 
            * downStandBeamsList: Updated input. 
            * AmatList: List of MultiFreedom contraint matrix of shape nConstraint x nDOFs. 
              nConstraint is 3 x number of downstandBeam nodes.
